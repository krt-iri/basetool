...
- **At create time, stamp the squadron** on the entity: derive it from the entity's owner (e.g. `ship.setOwningSquadron(user.getSquadron())`) when the aggregate has an owner, or from `SquadronScopeService.currentSquadron()` when it does not (Operation). For Job Order, set `creating_squadron_id` from the caller's context (immutable after) and `requesting_squadron_id` from the request DTO. Until Flyway `V86` lands the NOT NULL tightening, missing stamps land as `NULL` and silently bypass the filter — keep the stamp in sync.
- **Admin area is admin-only** (post-Phase-4 lockdown). Stammdaten (Cities, Materials, ShipTypes, Locations, UEX, etc.), user-management endpoints (role flags, rank, attribute patches), announcement writes, system settings, and the promotion-system administration are all `@PreAuthorize("hasRole('ADMIN')")`. Officers keep their squadron-internal capabilities (mission management, hangar write, refinery management, logistician role via hierarchy) but lose system-wide configuration access.
