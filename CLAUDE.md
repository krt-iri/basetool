...
- **Aggregate scope kinds:**
  - *Strict-staffel* (squadron-scoped, no cross-staffel escape): `Ship`, `InventoryItem` (direct Lager-View), `RefineryOrder`, `Operation`. Direct CRUD and list endpoints filter by `owning_squadron_id`; detail endpoints gate on `canSee*`/`canEdit*` from `SquadronScopeService`.
  - *Cross-staffel-with-public-escape*: `Mission`. Visible to other squadrons iff `is_internal = false`; editable only by the owning squadron + admins. The repository's `searchMissions` enforces this via the `owning = :ctx OR is_internal = false` clause.
  - *Cross-staffel workspace*: `JobOrder` + linked `JobOrderMaterial` + `JobOrderHandover`. **No squadron filter on access** — anyone with the role/permission can read or edit. Job Order carries two squadron refs: `creating_squadron_id` (immutable, who authored it) and `requesting_squadron_id` (editable, on whose behalf it runs). Inventory items linked to a job order via `job_order_id` surface cross-staffel inside the order's UI even when they belong to a different squadron's stock, but they NEVER leak into a foreign squadron's Lager-View (split repository methods enforce this — `findInventoryForSquadronLager` is gated, `findByJobOrderIdOrdered` is not).
- **At create time, stamp the squadron** on the entity: derive it from the entity's owner (e.g. `ship.setOwningSquadron(user.getSquadron())`) when the aggregate has an owner, or from `SquadronScopeService.currentSquadron()` when it does not (Operation). For Job Order, set `creating_squadron_id` from the caller's context (immutable after) and `requesting_squadron_id` from the request DTO. Until Flyway `V89` lands the NOT NULL tightening, missing stamps land as `NULL` and silently bypass the filter — keep the stamp in sync.
- **Admin area is admin-only** (post-Phase-4 lockdown), with one carve-out. Stammdaten (Cities, Materials, ShipTypes, Locations, UEX, etc.), user-management endpoints (role flags, rank, attribute patches), announcement writes, and system settings are all `@PreAuthorize("hasRole('ADMIN')")`. The **promotion-system maintenance** (Themenbereiche / Bewertungsverwaltung / Rangvoraussetzungen) is re-opened to OFFICER under a squadron-scope gate: an Officer of squadron X may manage X's criteria via `canEditSquadron(topic.owningSquadron.id)`. Admins can additionally **toggle the entire promotion subsystem per squadron** via `PATCH /api/v1/squadrons/{id}/promotion-enabled` (page: `/admin/settings`) — `SquadronScopeService.isPromotionFeatureEnabledForCurrentScope()` short-circuits every promotion service to empty / 403 for non-admins when the flag is OFF, admins always bypass it so they can re-enable a locked-out squadron without data loss. Officers keep their other squadron-internal capabilities (mission management, hangar write, refinery management, logistician role via hierarchy) but lose system-wide configuration access.
- **ArchUnit guard `staffelScopedServicesMustWireSquadronOrAuthHelper`** in [`ArchitectureTest`](backend/src/test/java/de/greluc/krt/iri/basetool/backend/ArchitectureTest.java) breaks the build if a service from the staffel-scoped whitelist stops injecting `AuthHelperService` / `SquadronScopeService`. Update the whitelist when you add a new staffel-scoped aggregate.
- **MDC field `squadronId`** is emitted by [`CorrelationIdFilter`](backend/src/main/java/de/greluc/krt/iri/basetool/backend/logging/CorrelationIdFilter.java) on every request alongside `correlationId` and `userId`. Logback patterns must include `%X{squadronId}` to keep audit trails intact.
