...
| **Phase 7 migrations `V86`–`V88`** | ❌ | Intentionally deferred. Only land them after at least one production deploy of V80–V85 (two-phase drop rule in `db/migration/README.md`). `V86` tightens NOT NULL on `app_user.squadron_id`, `owning_squadron_id` on the 5 aggregates + `promotion_topic`, and `creating_squadron_id` + `requesting_squadron_id` on `job_order`. `V87` removes the legacy `squadron` field from the JPA `JobOrder` entity and relaxes the NOT NULL on the column. `V88` drops the column. Do NOT do this yet — flag it as a separate PR after the next prod release. The handover audit columns (`executing_user_id`, `executing_squadron_id`) shipped as V84 and the promotion-topic squadron column as V85 in this release, so they are NOT part of the Phase 7 train. |
