...
- **Mission planning** — plan, brief and review squadron missions with role-aware access (`MISSION_MANAGER`, `OFFICER`, `ADMIN`), including a sortable *Ablauf* (procedure-step checklist) whose shared done-state and current phase are tracked live during a running mission. Non-internal missions are browsable (and joinable) without an account. The owning org unit a mission is assigned to can be changed after creation from the Verwaltung tab (or set to *ownerless*), re-scoping its visibility (ADR-0050). A mission detail page updates live for every viewer over the presence WebSocket, so a peer's change (join, crew move, finance entry, schedule/status/party-lead/frequency/Ablauf/owning-org-unit edit) propagates without a manual reload.
