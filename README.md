...
[ADR-0066](docs/adr/0064-transactional-email-delivery-channel.md)) **ships enabled**, but sends
...
| `MONITORING_SCRAPE_USER`                     | (Optional) Basic-auth username for the Prometheus scrape endpoint `/actuator/prometheus` (all three modules; REQ-OBS-005, ADR-0066). Fail-closed: while unset, the endpoint denies every request.                                                                                             | *(unset → endpoint denies all)*                                 |
...
| `MONITORING_TRACING_ENABLED`                 | (Optional) Master gate for distributed tracing (all three modules; REQ-OBS-009, ADR-0066). `false` keeps tracing fully inert — no tracer, no `traceparent` propagation, no exporter. Set `true` together with `MONITORING_OTLP_ENDPOINT` where the monitoring stack runs (epic #936 Phase 2). | `false`                                                         |
