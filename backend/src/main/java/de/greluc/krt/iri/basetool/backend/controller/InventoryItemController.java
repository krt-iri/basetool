...
public class InventoryItemController {
    ...
    public List<de.greluc.krt.iri.basetool.backend.model.dto.GroupedInventoryDto> getMyGroupedInventory(
        ...
        return inventoryItemService.getMyAggregatedInventory(userService.getUserIdFromJwt(jwt), materialIds, minQuality, jobOrderIds, missionIds);
    }
    ...
    public PageResponse<InventoryItemDto> getAllInventory(
            ...
            @RequestParam(required = false, defaultValue = "material.name,asc;quality,desc;amount,desc") String sort) {
        Pageable pageable = PaginationUtil.createPageRequest(page, size, sort, Set.of("amount", "quality", "id", "material.name"), "quality");
        Page<InventoryItemDto> p = inventoryItemService.getAllInventory(materialIds, minQuality, jobOrderIds, missionIds, pageable);
        return new PageResponse<>(p.getContent(), p.getNumber(), p.getSize(), p.getTotalElements(), p.getTotalPages(), PaginationUtil.toSortStrings(p.getSort()));
    }
    ...
    public List<de.greluc.krt.iri.basetool.backend.model.dto.GroupedInventoryDto> getAllGroupedInventory(
            ...
            @RequestParam(required = false) List<UUID> jobOrderIds,
            @RequestParam(required = false) List<UUID> missionIds) {
        return inventoryItemService.getAllAggregatedInventory(materialIds, minQuality, jobOrderIds, missionIds);
    }
     ...
     * Owner may always modify their own note; non-owners require LOGISTICIAN (or higher via role
    ...
    @Operation(summary = "Bulk checkout", description = "Removes all specified inventory items that belong to the authenticated user. Associations to job orders and missions are cleared before deletion.")
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "403", description = "Access denied – item belongs to another user"),
    ...
    public void bulkCheckout(
            @AuthenticationPrincipal Jwt jwt,
            @RequestBody @Valid BulkCheckoutRequest request) {
        inventoryItemService.bulkCheckout(request, userService.getUserIdFromJwt(jwt));
    }

    @Operation(summary = "Update delivered status", description = "Updates the delivered flag of an inventory item. Applies optimistic locking via the version field.")
    @ApiResponses({
