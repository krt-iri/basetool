...
public class LocationController {
    ...
    @GetMapping
    public PageResponse<LocationDto> getAllLocations(@RequestParam(required = false) Integer page,
    ...
    public LocationDto createLocation(@RequestBody @NotNull LocationDto location) {
        return locationMapper.toDto(locationService.createLocation(locationMapper.toEntity(location)));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('OFFICER', 'ADMIN')")
    public LocationDto updateLocation(@PathVariable @NotNull UUID id, @RequestBody @NotNull LocationDto location) {
