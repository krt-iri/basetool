...
/**
 * REST controller for the material collection overview of a job order.
 * Provides a sorted list of all inventory items linked to a specific job order.
 */
@RestController
...
public class MaterialCollectionController {
    ...
    @Operation(
            ...
            description = "Returns all inventory items linked to the given job order, sorted by owner name, location, material name, quality (desc), quantity (desc)."
    )
    @ApiResponses({
    ...
    public List<MaterialCollectionEntryDto> getMaterialCollection(@PathVariable @NotNull UUID jobOrderId) {
