...
@Tag(name = "Operations",
        description = "Operation aggregate: groups multiple missions under one umbrella and exposes "
...
public class OperationController {
    ...
    // hasRole('MISSION_MANAGER') below also matches users with app_user.is_mission_manager=true — CustomJwtGrantedAuthoritiesConverter injects ROLE_MISSION_MANAGER from the DB flag at JWT-decode time.

    @GetMapping
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "List all operations (paginated)",
            description = "Returns operations ordered by `sort` (default `createdAt,desc`). "
    ...
    public PageResponse<OperationDto> getAllOperations(
        ...
        Pageable pageable = PaginationUtil.createPageRequest(page, size, sort, ALLOWED_SORT, "createdAt");
        Page<OperationDto> dtoPage = operationService.getAllOperations(pageable).map(operationMapper::toDto);
    ...
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Get aggregated finances for an operation",
            description = "Sums income, expenses and refinery profit/loss across all missions "
    ...
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Get participation-time payout breakdown",
            description = "For each participant across all missions of the operation, returns the "
    ...
    public OperationDto createOperation(@Valid @RequestBody OperationCreateDto createDto) {
        de.greluc.krt.iri.basetool.backend.model.Operation operation = operationMapper.toEntity(createDto);
    ...
    @Operation(summary = "Update an existing operation",
            description = "Requires the current `version` field in the body for optimistic locking. "
    ...
    public OperationDto updateOperation(@PathVariable UUID id,
        ...
        boolean canOverrideStatus = authentication != null
                && authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .anyMatch("ROLE_ADMIN"::equals);
        return operationMapper.toDto(operationService.updateOperation(id, updateDto, canOverrideStatus));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete an operation",
            description = "Unlinks every mission that belongs to the operation (sets "
