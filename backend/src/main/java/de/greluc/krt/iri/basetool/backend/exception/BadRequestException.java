 ...
 * cross-field constraints not modellable as a Jakarta annotation, references to entities that
 * exist but are in the wrong state, or requests that conflict with an aggregate's current state.
 *
 ...
 * error code {@code BAD_REQUEST}. Prefer this over {@code ResponseStatusException} so the
