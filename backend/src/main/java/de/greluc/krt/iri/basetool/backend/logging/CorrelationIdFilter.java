 ...
 *
 * <p>Each request is decorated with two MDC keys:</p>
 * <ul>
 *   <li><b>correlationId</b> – either taken from the inbound header (configurable via
 *       {@link LoggingProperties#getCorrelationIdHeader()}) or freshly generated as UUID. The
 *       effective id is echoed back in the response header of the same name so clients/proxies
 *       can trace the same request end-to-end.</li>
 ...
 *       leaking PII (see AGENTS.md: no emails/names/tokens in logs).</li>
 * </ul>
 *
 * <p>The MDC is cleared in a {@code finally} block to prevent bleed-through on pooled or virtual
 * threads. The filter runs after Spring Security (order {@link Ordered#LOWEST_PRECEDENCE} minus
 * a small delta) so that {@link JwtAuthenticationToken} is already populated when the MDC is set.
 * A secondary lightweight pass at filter start still generates / echoes the correlation id even
 * for unauthenticated requests, ensuring every log line has the same id.</p>
 */
@Slf4j
@Component
...
public class CorrelationIdFilter extends OncePerRequestFilter implements Ordered {
    ...
    /**
     * Run very late in the servlet filter chain so Spring Security has already populated the
     * {@link SecurityContextHolder}. Using {@link Ordered#LOWEST_PRECEDENCE} minus a constant lets
