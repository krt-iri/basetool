 ...
 * phone number, Keycloak {@code sub}), because it keeps the information redacted even if the
 * layout is ever replaced or reconfigured.
 *
 * <p>All methods are null-safe and never throw on blank / malformed input – they simply return
 * a placeholder so that log statements do not risk NPEs.
 */
public final class LogMasker {
    ...
    /**
     * Masks an e-mail address so that only the first character of the local part and the full
     * domain remain, e.g. {@code "alice@example.com" -> "a***@example.com"}.
     *
    ...
    /**
     * Masks an identifier (database PK, Keycloak {@code sub}, UUID, correlation-id, ...) so that
     * only the first and last two characters remain, e.g. {@code "123456789" -> "12***89"}.
     *
    ...
    /**
     * Masks any token-like secret (JWT, API key, session id). The result never contains more
     * than the first 4 characters plus a fixed suffix, to keep the log useful for correlation
     * while not leaking the secret.
     */
    @Contract(pure = true)
    ...
    /**
     * Masks a phone number so that only the last two digits remain, e.g.
     * {@code "+49 170 1234567" -> "***67"}. Non-digit characters are stripped before masking.
     */
    @Contract(pure = true)
    ...
    /**
     * Generic fallback – fully hides the value but keeps length information
     * (useful to detect truncation/encoding issues without leaking content).
     */
    @Contract(pure = true)
