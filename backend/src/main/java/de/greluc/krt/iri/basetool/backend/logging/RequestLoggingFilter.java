 ...
 *
 * <p>The line includes HTTP method, path, response status and elapsed duration. Correlation id
 * and user id are intentionally not duplicated into the message body because they are already
 * rendered via the MDC pattern in {@code logback-spring.xml}. This avoids noisy, redundant log
 * output and keeps structured-JSON fields clean.</p>
 *
 * <p>Slow requests (duration above {@link LoggingProperties#getSlowRequestThresholdMs()}) are
 * logged at WARN so operators can flag them in dashboards / alerting.</p>
 *
 * <p>Runs slightly earlier than {@link CorrelationIdFilter} in terms of filter order but since
 * both filters run once per request, the MDC values set by the correlation filter are still
 * available when this filter logs in its {@code finally} block – the correlation filter wraps
 * this one via the servlet chain.</p>
 */
@Slf4j
@Component
