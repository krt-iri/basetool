...
public class JobOrder extends AbstractEntity<UUID> {
   ...
   * during the two-phase migration; Flyway V85 removes the JPA field and V86 drops the DB column.
   * New read paths must use {@link #requestingSquadron} instead - this field stays mirrored only to
   * keep the legacy {@code NOT NULL} column populated until V85 lifts the constraint.
   */
