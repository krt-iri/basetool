...
/**
 * Patch payload for the per-membership role flags on
 * {@link de.greluc.krt.iri.basetool.backend.model.OrgUnitMembership}. Endpoint
 * {@code PATCH /api/v1/special-commands/{id}/members/{userId}} accepts this record so the admin /
 * SK-Lead UI can flip one or both flags in a single round-trip without touching the others.
 *
 ...
 * silently overwriting each other. {@code is_lead} is intentionally NOT part of this payload —
 * Lead toggles ship through a dedicated ADMIN-only endpoint ({@code PATCH .../lead}) so the audit
 * trail can clearly attribute "X promoted Y to Lead of SK Z" to a specific admin action.
 *
