 ...
 * embedded composite key unpacked as two separate UUID fields ({@link #userId},
 * {@link #orgUnitId}) plus a denormalised {@link #userDisplayName} so the admin roster page can
 * render the member chip without a per-row join back to {@code app_user}. The {@code kind} field
 * mirrors {@link OrgUnitKind} so the client can branch on Squadron vs Spezialkommando without a
 * follow-up lookup against the parent {@code org_unit} row.
 *
 ...
 * @param userDisplayName Convenience copy of {@code user.displayName} so the admin roster does not
 *     need a per-row join.
 ...
 *     org unit. Once R5.c migrates the scoped-role authorisation onto the membership row, this is
 *     the authoritative source for "may this user perform a Logistician action in this org unit?".
 ...
 *     {@code false} on Squadron memberships by the V95 CHECK constraint
 *     {@code chk_org_unit_membership_lead_only_on_special_command}.
