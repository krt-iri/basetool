 ...
 * plus a denormalised {@link #userDisplayName} so the admin roster page can render the member chip
 * without a per-row join back to {@code app_user}. The {@code kind} field mirrors {@link
 * OrgUnitKind} so the client can branch on Squadron vs Spezialkommando without a follow-up lookup
 * against the parent {@code org_unit} row.
 *
