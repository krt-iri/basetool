...
public class JobOrderHandoverReportService {
     ...
     * from the {@code X-User-Time-Zone} request header). If {@code userZone} is {@code null},
    ...
    public byte @NotNull [] generateHandoverReport(@NotNull UUID jobOrderId, @NotNull UUID handoverId,
        ...
        List<ItemRow> rows = handover.getItems().stream()
                .map(item -> new ItemRow(
                        ...
                        item.getMaterial() != null && item.getMaterial().getQuantityType() != null
                                ? item.getMaterial().getQuantityType().name() : null
                ))
     ...
     * @return PDF as byte array
     */
    public byte @NotNull [] generateHandoverReportPreview(@NotNull HandoverReportPreviewRequestDto dto) {
        ...
        List<ItemRow> rows = dto.items().stream()
                .map(item -> new ItemRow(
                        item.materialName(),
    ...
    private static final Comparator<ItemRow> ITEM_COMPARATOR = Comparator
            .comparing(ItemRow::materialName, String.CASE_INSENSITIVE_ORDER)
    ...
    private void addTableCell(@NotNull PdfPTable table, @NotNull String text, @NotNull Color bg, boolean centered) {
    ...
    /**
     * PdfPageEvent that paints the KRT corporate-design background on every page,
     * ensuring pages 2+ receive the same dark background, orange accent bars and logo
     * as the first page.
     */
    private class KrtPageBackground extends PdfPageEventHelper {
        ...
        public void onStartPage(PdfWriter writer, Document document) {
            ...
            try (InputStream logoStream = getClass().getClassLoader().getResourceAsStream("META-INF/resources/logos/staffel_iridium.png")) {
                if (logoStream != null) {
