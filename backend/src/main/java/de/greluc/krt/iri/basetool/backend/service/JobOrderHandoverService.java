...
public class JobOrderHandoverService {
     ...
     * <ul>
     *   <li>reduces inventory amounts (or deletes the row when fully consumed),</li>
     *   <li>reduces the open amount per {@link de.greluc.krt.iri.basetool.backend.model.JobOrderMaterial},</li>
     *   <li>persists the handover plus its items,</li>
     *   <li>unlinks remaining inventory rows for materials that are now fully fulfilled,</li>
     *   <li>completes the JobOrder if all materials are fulfilled.</li>
     * </ul>
     *
     * <p><b>Concurrency / Optimistic Locking:</b> The previous implementation issued the bulk
     * {@code unlinkJobOrderMaterial} update <em>inside</em> the per-item loop. The bulk update
     * carries {@code @Modifying(clearAutomatically = true, flushAutomatically = true)} which
     * (1) flushes pending changes mid-iteration and (2) detaches the {@link JobOrder} aggregate
     * (and all of its {@link de.greluc.krt.iri.basetool.backend.model.JobOrderMaterial}s) from
     * the persistence context. Subsequent loop iterations then operated on detached entities and
     * called {@code jobOrderMaterialRepository.save(mat)} which silently triggers an
     * {@code EntityManager.merge()}. Combined with the cascade on {@code JobOrder.materials} and
     * the additional {@code findById}/{@code save}/{@code flush} cycle in
     * {@code JobOrderService.completeJobOrderWithinTransaction()} this produced a second
     * version-bump on already-modified rows — a textbook
     * {@link org.springframework.orm.ObjectOptimisticLockingFailureException} (HTTP 409) when an
     * entire JobOrder was fulfilled by a single multi-material handover.
     *
     * <p>The structural fix (extension of the {@code *WithinTransaction} pattern documented in
     * {@code AGENTS.md} to the {@code JobOrderMaterial} / {@code Stock} aggregates) consists of
     * three rules enforced below:
     * <ol>
     *   <li>The loop only mutates managed entities — it relies on Hibernate's dirty checking
     *       and never calls {@code jobOrderMaterialRepository.save(mat)} explicitly.</li>
     *   <li>Bulk updates with {@code clearAutomatically=true} are deferred until <em>after</em>
     *       the loop has completed and the new handover has been persisted.</li>
     *   <li>The {@link JobOrder} is re-fetched <em>once</em> after the bulk updates so the
     *       completion check operates on a freshly managed aggregate (with up-to-date
     *       {@code @Version}s).</li>
     * </ol>
     */
    @Transactional
    public JobOrderHandoverDto createHandover(UUID jobOrderId, JobOrderHandoverCreateDto dto) {
        ...
        for (JobOrderHandoverItemCreateDto itemDto : dto.items()) {
            ...
            handoverItem.setLocationName(inventoryItem.getLocation() != null ? inventoryItem.getLocation().getName() : null);
