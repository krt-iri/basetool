...
public class JobOrderService {
   ...
   */
  /**
   * Transitions a job order's status. Backend enforces the state machine (OPEN → IN_PROGRESS →
   * COMPLETED, OPEN → REJECTED) and rejects illegal transitions with a 400.
   *
   * @param id job order primary key
   ...
   */
  /**
   * Canonical example of the {@code …WithinTransaction} pattern (see CLAUDE.md Concurrency).
   *
   * <p>Marks a job order as complete when all of its material requirements have been satisfied.
   * This method must be called from inside an existing transaction ({@code propagation =
   * MANDATORY}) and operates on the already-managed {@code jobOrder} entity without {@code
   * save()}/{@code flush()}: Hibernate's dirty-checking emits the UPDATE at transaction commit.
   *
   * <p>Without this pattern, a {@code save()} here would re-merge the entity, bumping
   * {@code @Version} a second time and triggering a 409 for the outer caller that already
   * incremented the version implicitly via cascaded saves. Every handover / booking / transfer flow
   * that needs to complete a job order routes through this method.
   *
   * @param jobOrder the already-managed job order entity
   */
