 ...
 * types and integer amounts for {@code PIECE}. {@code SCU} amounts are <em>not</em> rejected for
 * excess precision — more than three decimals is commercially rounded (HALF_UP) to three places at
 * the persistence boundary, mirroring the frontend. {@code null} fields are intentionally accepted
 * here — the {@code @NotNull} annotations on the underlying DTO fields are responsible for their
 * own reporting. Annotate at the type level rather than per field so the validator can see {@code
 * materialId} and {@code amount} together.
 */
@Documented
