...
/**
 * Validator backing {@link WholeNumber}: a {@link BigDecimal} is valid when it has no fractional
 * part. Wholeness is tested by value (remainder modulo one is zero), not by scale, so {@code
 * 500.00} passes while {@code 500.50} fails. {@code null} passes — null-ness is enforced separately
 * by {@code @NotNull}.
 */
