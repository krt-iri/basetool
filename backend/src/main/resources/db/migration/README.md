...
- **V80–V93** — Multi-Squadron-Umbau (see [`MULTI_SQUADRON_PLAN.md`](../../../../../../../../MULTI_SQUADRON_PLAN.md)).
  ...
  VARCHAR in V90.
- **V94–V98** — Spezialkommando-Erweiterung (see `SPEZIALKOMMANDO_PLAN.md`).
  ...
  stages:
  - **V94** — create `org_unit` with `kind IN ('SQUADRON','SPECIAL_COMMAND')`,
    copy every existing `squadron` row in as `kind='SQUADRON'`, add the
    promotion-only-for-Squadron CHECK.
  - **V95** — create `org_unit_membership` (composite PK, denormalised `kind`
  ...
  - **V96** — add nullable `owning_org_unit_id` columns + FKs + indexes on
  ...
  - **V97** — one-way sync trigger that mirrors INSERT/UPDATE/DELETE on
  ...
  - **V98** — promotion-topic SK-reject trigger (`guard_promotion_topic_owner_kind`)
...
- **V99+ (destructive cleanup, deferred)** — tighten `owning_org_unit_id` to
  ...
  drop the legacy `squadron` table once V97's mirror is no longer needed.
