...
-- org_unit instead. The V97 sync trigger + function that mirrored org_unit ->
...
-- the constraint names + the V98 kind-guard trigger stay readable per
-- Plan §3.3.
...
-- 4. Drop the V97 sync trigger + function. With no foreign keys left pointing
-- at squadron(id), the mirror table has no consumers -- its only purpose was
-- to keep the dropped FKs resolving while the V96 / V99 / V100 / V101 / V102
-- chain rolled out.
...
-- dropped in earlier migrations (V100 dropped the aggregate FKs, V101
