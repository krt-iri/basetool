 ...
 *       {@link #controllerLayerShouldNotReachIntoSecurityContext()} and
 *       {@link #mapperLayerShouldNotReachIntoSecurityContext()} (no {@code SecurityContextHolder}
 *       outside the dedicated auth-helper services) plus
...
class ArchitectureTest {
    ...
    /**
     * Java-generic wrappers that controllers legitimately return (paging envelopes, optional
     * results, response wrappers). The Entity-Generic rule below scans the actual type
     * arguments of these wrappers to make sure a JPA {@code @Entity} never leaks through.
     */
    private static final Set<String> ENTITY_GENERIC_WRAPPERS = Set.of(
    ...
    /**
     * Method-name prefixes that the codebase uses for state-mutating service operations.
     * Used by {@link #mutatingServiceMethodsInReadOnlyClassesNeedExplicitTransactional()} to
     * find methods that must override a class-level {@code @Transactional(readOnly = true)}
     * with their own {@code @Transactional}. The list is conservative — anything that does
     * NOT start with one of these prefixes is treated as a read operation.
     */
    private static final Set<String> MUTATING_METHOD_PREFIXES = Set.of(
            "create", "update", "delete", "add", "remove", "save", "store",
            "book", "handover", "link", "unlink", "move", "reset", "patch",
            "complete", "approve", "reject", "publish", "cancel", "join",
            "leave", "register", "unregister", "set", "insert", "merge",
            "assign", "unassign", "increment", "decrement", "clear", "purge",
            "import", "sync"
    );

    /**
     * Classes that are allowed to reach into {@link org.springframework.security.core.context.SecurityContextHolder}
     * despite being on the service-layer package. By design the list contains exactly
     * one entry — {@code AuthHelperService} — so there is a single source of truth
     * for "what is the current authentication" across the codebase.
     */
    ...
    void controllerLayerShouldNotReachIntoSecurityContext() {
        ...
        noClasses()
                ...
                .should().dependOnClassesThat().haveFullyQualifiedName(SECURITY_CONTEXT_HOLDER)
                .because("Controllers must read the principal via @AuthenticationPrincipal / "
    ...
    void mapperLayerShouldNotReachIntoSecurityContext() {
        ...
        noClasses()
                .that().resideInAPackage("..backend.mapper..")
                .should().dependOnClassesThat().haveFullyQualifiedName(SECURITY_CONTEXT_HOLDER)
                .because("Mappers must stay pure transformers; route any auth lookup through "
    ...
    void controllerLayerShouldNotDependOnRepositoryLayer() {
        ...
        noClasses()
                .that().resideInAPackage("..backend.controller..")
                .should().dependOnClassesThat().resideInAPackage("..backend.repository..")
                .because("Controllers must go through the service layer — a controller depending on a "
    ...
    void controllerMethodsShouldNotExposeJpaEntitiesInGenericWrappers() {
        ...
        methods()
                ...
                .because("Controllers must wrap DTOs, never JPA entities, even when the entity is "
    ...
    void mutatingServiceMethodsInReadOnlyClassesNeedExplicitTransactional() {
        ...
        classes()
                ...
                .because("A class-level @Transactional(readOnly = true) silently propagates to every "
    ...
    void writeEndpointsMustDeclareAnAuthorisationAnnotation() {
        ...
        methods()
                ...
                .because("Every state-changing HTTP endpoint must carry an explicit @PreAuthorize "
                ...
                for (JavaType arg : parameterized.getActualTypeArguments()) {
                    ...
                    if (argClass.isAnnotatedWith(JPA_ENTITY)) {
                        events.add(SimpleConditionEvent.violated(method,
                                method.getFullName() + " returns " + rawType.getSimpleName()
                                        + "<" + argClass.getSimpleName() + "> — JPA entities must not "
    ...
    private static ArchCondition<JavaClass> declareTransactionalForMutatingMethodsWhenClassIsReadOnly() {
        return new ArchCondition<JavaClass>(
            ...
            public void check(JavaClass clazz, ConditionEvents events) {
                ...
                for (JavaMethod method : clazz.getMethods()) {
                    ...
                    events.add(SimpleConditionEvent.violated(method,
                            method.getFullName() + " — declaring class is @Transactional(readOnly = true) "
    ...
    private static ArchCondition<JavaMethod> haveMethodOrClassLevelPreAuthorize() {
        return new ArchCondition<JavaMethod>("declare @PreAuthorize on the method or on the declaring class") {
            @Override
            public void check(JavaMethod method, ConditionEvents events) {
                if (method.isAnnotatedWith(PRE_AUTHORIZE)) {
                    return;
                ...
                events.add(SimpleConditionEvent.violated(method,
                        method.getFullName() + " — state-changing endpoint without @PreAuthorize. "
    ...
    private static boolean isClassReadOnlyTransactional(JavaClass clazz) {
        ...
        return annotation.tryGetExplicitlyDeclaredProperty("readOnly")
