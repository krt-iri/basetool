...
import de.greluc.krt.iri.basetool.backend.model.dto.AddFinanceEntryRequest;
...
class FeatureExpansionTest {
    ...
    void testLocationCrud_Officer_Allowed() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/locations/" + saved.getId())
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER")))
                .contentType(MediaType.APPLICATION_JSON)
        ...
        // Delete
        mockMvc.perform(delete("/api/v1/locations/" + saved.getId())
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER"))))
                .andExpect(status().isOk());
    ...
    void testShipWithLocation() throws Exception {
        ShipRequestDto req = new ShipRequestDto("Located Ship", fighter.getId(), "LTI", stanton.getId(), false, null);
        
        // Use HangarController to add ship
        String response = mockMvc.perform(post("/api/v1/hangar/ships")
                .with(jwt().jwt(builder -> builder.subject(normalUser.getId().toString()))
    ...
    void testSubMission() throws Exception {
        ...
        String subJson = String.format("{\"name\": \"Sub Mission\", \"status\": \"PLANNED\", \"version\": 0}");

        String response = mockMvc.perform(post("/api/v1/missions/" + parent.getId() + "/sub-missions")
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(subJson))
        ...
        de.greluc.krt.iri.basetool.backend.model.dto.MissionDto savedSub = objectMapper.readValue(response, de.greluc.krt.iri.basetool.backend.model.dto.MissionDto.class);
        ...
        // Since we are in same transaction and JPA caching, refresh might be needed, or trust the test context.
        // But subMissions list on parent side is not automatically updated in memory unless we add it or refresh.
        // The service added it to parent? In addSubMission: "subMission.setParent(parent); return missionRepository.save(subMission);"
    ...
    void testMissionFinance() throws Exception {
        ...
        de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryCreateDto req = new de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryCreateDto(
                mission.getId(), participant.getId(), "Fuel", FinanceType.EXPENSE, new BigDecimal("50.00")
        );

        String response = mockMvc.perform(post("/api/v1/finance-entries")
                .with(jwt().jwt(builder -> builder.subject(normalUser.getId().toString()))
        ...
        de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryDto entryDto = objectMapper.readValue(response, de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryDto.class);
        assertEquals(new BigDecimal("50.00"), entryDto.amount());
        assertEquals(FinanceType.EXPENSE, entryDto.type());
        
        // Remove
        mockMvc.perform(delete("/api/v1/finance-entries/" + entryDto.id())
                .with(jwt().jwt(builder -> builder.subject(normalUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_SQUADRON_MEMBER"))))
    ...
    void testMissionFinance_OtherUser_Forbidden() throws Exception {
        ...
        // otherUser is not a participant of this mission, so they should get Forbidden when trying to create an entry for normalUser, or even themselves.
        // Actually, the check says "must be a registered participant of this mission".
        // Wait, creating entry has check? The `createEntry` logic doesn't explicitly check if the creator is a participant! It just assigns the `dto.userId()`.
        // The requirements say "alle nutzer (auch nicht angemeldete) können neue einnahmen und ausgaben erfassen."
        ...
        de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryCreateDto reqOther = new de.greluc.krt.iri.basetool.backend.model.dto.MissionFinanceEntryCreateDto(
                mission.getId(), otherParticipant.getId(), "Snacks", FinanceType.EXPENSE, new BigDecimal("10.00")
        );
        
        String response = mockMvc.perform(post("/api/v1/finance-entries")
                .with(jwt().jwt(builder -> builder.subject(otherUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_SQUADRON_MEMBER")))
                .contentType(MediaType.APPLICATION_JSON)
