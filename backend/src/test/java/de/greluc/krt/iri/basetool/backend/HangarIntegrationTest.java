...
class HangarIntegrationTest {
    ...
    void testUserManageOwnHangar() throws Exception {
        // Add Ship
        ShipRequestDto shipReq = new ShipRequestDto("My Fighter", fighter.getId(), "LTI", null, true, null);
        ...
        // Update Ship
        ShipRequestDto updateReq = new ShipRequestDto("Updated Fighter", fighter.getId(), "LTI", null, true, savedShip.version());
        mockMvc.perform(put("/api/v1/hangar/ships/" + savedShip.id())
                .with(jwt().jwt(builder -> builder.subject(user1.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("HANGAR_WRITE")))
                .contentType(MediaType.APPLICATION_JSON)
        ...
        // Delete Ship
        mockMvc.perform(delete("/api/v1/hangar/ships/" + savedShip.id())
                .with(jwt().jwt(builder -> builder.subject(user1.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("HANGAR_WRITE"))))
                .andExpect(status().isOk());
    ...
    void testUserCannotManageOtherHangar() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/hangar/ships/" + ship.getId())
        ...
        mockMvc.perform(delete("/api/v1/hangar/ships/" + ship.getId())
                .with(jwt().jwt(builder -> builder.subject(user2.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("HANGAR_WRITE"))))
                .andExpect(status().isForbidden());
    }
    ...
    void testAdminManageUserHangar() throws Exception {
        ...
        ShipRequestDto req = new ShipRequestDto("Admin Gift", fighter.getId(), "LTI", null, false, null);

        String response = mockMvc.perform(post("/api/v1/hangar/users/" + user1.getId() + "/ships")
        ...
        // Admin updates ship
        ShipRequestDto upReq = new ShipRequestDto("Admin Updated", fighter.getId(), "LTI", null, false, savedShip.version());
        mockMvc.perform(put("/api/v1/hangar/users/" + user1.getId() + "/ships/" + savedShip.id())
                .with(jwt().jwt(builder -> builder.subject(adminUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_ADMIN"), new SimpleGrantedAuthority("ROLE_MANAGE"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(upReq)))
        ...
        // Admin deletes ship
        mockMvc.perform(delete("/api/v1/hangar/users/" + user1.getId() + "/ships/" + savedShip.id())
                .with(jwt().jwt(builder -> builder.subject(adminUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_ADMIN"), new SimpleGrantedAuthority("ROLE_MANAGE"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE"))))
                .andExpect(status().isOk());
    ...
    void testAdminResetAllFittedStatus() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/hangar/ships/reset-fitted")
                .with(jwt().jwt(builder -> builder.subject(adminUser.getId().toString()))
    ...
    void testSquadronOverview() throws Exception {
        ...
        String response = mockMvc.perform(get("/api/v1/hangar/squadron-overview")
                .with(jwt().jwt(builder -> builder.subject(user1.getId().toString()))
    ...
    void testSquadronOverviewAsAdmin() throws Exception {
        ...
        String response = mockMvc.perform(get("/api/v1/hangar/squadron-overview")
                .with(jwt().jwt(builder -> builder.subject(adminUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_ADMIN"), new SimpleGrantedAuthority("HANGAR_READ"))))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
