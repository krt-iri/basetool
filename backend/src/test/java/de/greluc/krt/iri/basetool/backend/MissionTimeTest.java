...
class MissionTimeTest {
    ...
    void testUpdateMissionTimes_Officer_Allowed() throws Exception {
        Instant start = Instant.now().minus(1, ChronoUnit.HOURS).truncatedTo(java.time.temporal.ChronoUnit.MILLIS);
        Instant end = Instant.now().plus(1, ChronoUnit.HOURS).truncatedTo(java.time.temporal.ChronoUnit.MILLIS);
