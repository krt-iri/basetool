...
import de.greluc.krt.iri.basetool.backend.controller.MissionController;
...
class MissionUnitFrequencyTest {
    ...
    void testAddUnitWithValidFrequency() throws Exception {
        String requestJson = String.format("{\"shipTypeId\": \"%s\", \"shipId\": \"%s\", \"frequency\": 100.00, \"name\": \"Unit Alpha\"}", shipType.getId(), ship.getId());

        mockMvc.perform(post("/api/v1/missions/" + mission.getId() + "/units")
    ...
    void testAddUnitWithInvalidFrequencyLow() throws Exception {
        String requestJson = String.format("{\"shipTypeId\": \"%s\", \"shipId\": \"%s\", \"frequency\": 99.99, \"name\": \"Unit Beta\"}", shipType.getId(), ship.getId());
    ...
    void testAddUnitWithInvalidFrequencyHigh() throws Exception {
        String requestJson = String.format("{\"shipTypeId\": \"%s\", \"shipId\": \"%s\", \"frequency\": 1000.00, \"name\": \"Unit Gamma\"}", shipType.getId(), ship.getId());
    ...
    void testAddUnitWithoutShip() throws Exception {
        // shipId is null
        String requestJson = String.format("{\"shipTypeId\": \"%s\", \"shipId\": null, \"frequency\": 150.00, \"name\": \"Unit Delta\"}", shipType.getId());
