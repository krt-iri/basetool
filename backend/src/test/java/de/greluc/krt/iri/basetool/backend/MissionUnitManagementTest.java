...
class MissionUnitManagementTest {
    ...
    void setUp() {
        ...
        mission = missionService.addUnitToMission(mission.getId(), "Initial Unit", st.getId(), ship.getId(), false, 123.45);
    ...
    void testUpdateUnit_Officer_Allowed() throws Exception {
        String updateJson = String.format(
            ...
            unit.getShipType().getId(),
            ship.getId()
        );

        mockMvc.perform(put("/api/v1/missions/" + mission.getId() + "/units/" + unit.getId())
    ...
    void testDeleteUnit_Officer_Allowed() throws Exception {
        mockMvc.perform(delete("/api/v1/missions/" + mission.getId() + "/units/" + unit.getId())
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE"))))
                .andExpect(status().isOk());

        Mission updatedMission = missionRepository.findById(mission.getId()).orElseThrow();
        assertFalse(updatedMission.getAssignedUnits().stream().anyMatch(u -> u.getId().equals(unit.getId())));
    }
}