...
class StarSystemTest {
    ...
    void testCreateStarSystem_Officer_Allowed() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/v1/star-systems")
    ...
    void testCreateStarSystem_Guest_Forbidden() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/star-systems")
    ...
    void testUpdateStarSystem_Officer_Allowed() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/star-systems/" + system.getId())
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE")))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(system)))
    ...
    void testDeleteStarSystem_Officer_Allowed() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/star-systems/" + system.getId())
                .with(jwt().jwt(builder -> builder.subject(officerUser.getId().toString()))
                        .authorities(new SimpleGrantedAuthority("ROLE_OFFICER"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE"))))
                .andExpect(status().isOk());
