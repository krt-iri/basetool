 ...
 *
 * <p>CLAUDE.md mandates "store/process as {@code Instant} or {@code OffsetDateTime}" plus
 * "write serialization tests for timezone behavior". These tests pin down the contract:
 * <ul>
 *   <li>{@link Instant} is always rendered as a UTC string ending in {@code Z}, never with
 *       a server-default offset.</li>
 *   <li>{@link OffsetDateTime} <em>serialises</em> with whichever offset the in-memory
 *       value carries (positive, negative or {@code Z}) — but Jackson's default
 *       {@code ADJUST_DATES_TO_CONTEXT_TIME_ZONE} setting <em>normalises the value to UTC
 *       on deserialise</em>. The latter matches the CLAUDE.md "times in UTC" contract:
 *       no matter what offset a client sends, the server-side instant is stored in UTC
 *       and the original offset is dropped.</li>
 *   <li>{@link LocalDateTime} in {@link HandoverReportPreviewRequestDto} is the documented
 *       exception (see DTO Javadoc): the value the user typed in their local time zone is
 *       echoed back unchanged, with no implicit conversion to UTC. The DTO is parsed and
 *       re-serialised here to make sure no Jackson default silently adds an offset.</li>
 *   <li>DST boundary timestamps (Europe/Berlin spring-forward) survive round-tripping
 *       through {@link OffsetDateTime} with their wall-clock value intact.</li>
 * </ul>
 *
 * <p>All tests configure the {@link ObjectMapper} the same way Spring Boot does by default
 * ({@code JavaTimeModule} registered, {@code WRITE_DATES_AS_TIMESTAMPS} disabled) so that
 * any regression in this configuration here also surfaces in production.
 */
class TimezoneSerializationTest {
    ...
    void instant_deserializesFromIsoStringWithNanos() throws Exception {
        ...
        assertThat(parsed.getEpochSecond()).isEqualTo(Instant.parse("2026-03-03T12:00:00Z").getEpochSecond());
    ...
    void handoverReportPreviewDto_keepsLocalDateTimeWithoutOffsetAcrossRoundTrip() throws Exception {
        ...
        // When
        HandoverReportPreviewRequestDto dto = objectMapper.readValue(json, HandoverReportPreviewRequestDto.class);
    ...
    @Test
    void handoverReportPreviewDto_acceptsLocalDateTimeWithoutOffsetEvenWhenServerInUtc() throws Exception {
        ...
        HandoverReportPreviewRequestDto roundTripped = objectMapper.readValue(json, HandoverReportPreviewRequestDto.class);
    ...
    void offsetDateTime_dstSpringForwardBoundary_preservesAbsoluteMomentInUtc() throws Exception {
        ...
        // When
        OffsetDateTime beforeParsed = objectMapper.readValue(
                objectMapper.writeValueAsString(beforeDst), OffsetDateTime.class);
        OffsetDateTime afterParsed = objectMapper.readValue(
                objectMapper.writeValueAsString(afterDst), OffsetDateTime.class);
