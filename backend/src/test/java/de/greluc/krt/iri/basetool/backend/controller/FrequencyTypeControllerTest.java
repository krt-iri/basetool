...
/**
 * Pure-method unit tests for {@link FrequencyTypeController}. The Spring-MVC
 * binding (path params, `@PreAuthorize`, JSON serialisation) is covered by
 * the integration suite; here we focus on:
 * <ul>
 *   <li>Pagination wrapper: the {@link PageResponse} is assembled from the
 *       Spring {@link Page} correctly (page / size / total / sort).</li>
 *   <li>Service-layer delegation: every CRUD action delegates with the
 *       expected arguments and the {@link FrequencyTypeMapper} round-trips
 *       in both directions.</li>
 *   <li>Activate / reorder endpoints are void-returning and do not silently
 *       swallow the id list.</li>
 * </ul>
 */
...
class FrequencyTypeControllerTest {
    ...
    void getAll_buildsPageableAndMapsContent() {
        ...
        // When
        PageResponse<FrequencyTypeDto> response = controller.getAllFrequencyTypes(null, null, null, true);
    ...
    void create_roundTripsDtoToEntityViaMapperAndBack() {
        ...
        FrequencyTypeDto response = new FrequencyTypeDto(UUID.randomUUID(), "Recon", "scout chatter", true, 5, 1L);
