...
/**
 * Pure-method unit tests for {@link LocationController}. Key behaviour
 * documented in the test names:
 * <ul>
 *   <li>The POST create endpoint must pass through
 *       {@link LocationMapper#stripServerManaged(Location)} so a client
 *       cannot mass-assign onto an existing row via {@code id} / {@code version}.</li>
 *   <li>The "lookup" endpoint returns reference DTOs directly from the service
 *       — no mapper involvement.</li>
 *   <li>Pagination wrapping ({@link PageResponse}) honours the
 *       {@code includeHidden} flag verbatim.</li>
 * </ul>
 */
...
class LocationControllerTest {
    ...
    void lookup_returnsReferenceListDirectlyFromService_noMapper() {
        ...
        List<LocationReferenceDto> refs = List.of(
