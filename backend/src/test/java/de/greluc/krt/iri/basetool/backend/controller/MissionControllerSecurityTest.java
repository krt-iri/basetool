...
class MissionControllerSecurityTest {
    ...
    void updateCrew_WithOfficerRole_ShouldReturn200() throws Exception {
        ...
        when(missionSecurityService.canManageMission(any(UUID.class), ArgumentMatchers.any())).thenReturn(true);
        when(missionService.updateCrewInShip(any(), any(), any(), any())).thenReturn(new Mission());

        mockMvc.perform(put("/api/v1/missions/{id}/units/{unitId}/crew/{crewId}", missionId, unitId, crewId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"jobTypeIds\": [\"" + UUID.randomUUID() + "\"]}")
                .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_OFFICER"), new SimpleGrantedAuthority("USER_MANAGE"), new SimpleGrantedAuthority("MISSION_MANAGE"), new SimpleGrantedAuthority("HANGAR_MANAGE"), new SimpleGrantedAuthority("REFINERY_MANAGE"))))
    ...
    void updateCrew_WithSquadronMemberRole_ShouldReturn403() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/missions/{id}/units/{unitId}/crew/{crewId}", missionId, unitId, crewId)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"jobTypeIds\": [\"" + UUID.randomUUID() + "\"]}")
                .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_SQUADRON_MEMBER"), new SimpleGrantedAuthority("HANGAR_READ"), new SimpleGrantedAuthority("HANGAR_WRITE"), new SimpleGrantedAuthority("MISSION_READ"), new SimpleGrantedAuthority("REFINERY_READ"), new SimpleGrantedAuthority("REFINERY_WRITE"))))
    ...
    void addManager_WithMissionManagerRole_ShouldReturn200() throws Exception {
        ...
        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.post("/api/v1/missions/{id}/managers/{userId}", missionId, userId)
