...
/**
 * Pure-method unit tests for {@link TerminalController}. The PUT endpoint
 * has a non-obvious contract that's documented inline in the controller:
 * even though it accepts a full {@link TerminalDto}, the service only
 * applies the {@code hidden} flag — a regression that started writing the
 * full DTO would unexpectedly let admins rename / re-link UEX-imported
 * terminals via this endpoint.
 */
...
class TerminalControllerTest {
    ...
    void getAllTerminals_wrapsServicePageIntoPageResponse() {
        ...
        TerminalDto dto = new TerminalDto(UUID.randomUUID(), "TDD A18", null, "Stanton", "ArcCorp", "Area18", null, false);
    ...
    void updateTerminal_onlyPropagatesHiddenFlag_notFullDto() {
        ...
        UUID id = UUID.randomUUID();
        TerminalDto request = new TerminalDto(
                id, "Renamed", "Override", "FakeSystem", "FakePlanet", "FakeCity", "FakeStation", true);
        Terminal updated = new Terminal();
        TerminalDto response = new TerminalDto(id, "OriginalName", null, "Stanton", "ArcCorp", "Area18", null, true);
