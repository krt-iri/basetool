...
/**
 * Unit tests for the {@link UexClient}. Uses {@link MockWebServer} to drive
 * the WebClient against an in-process HTTP endpoint instead of hitting the
 * real UEX API.
 *
 * <p>Three behaviour patterns to cover for every endpoint method:
 * <ol>
 *   <li>Happy path — JSON wrapped in {@code {"status":"ok","data":[...]}} →
 *       list contents are parsed and returned.</li>
 *   <li>Server error (5xx, network blip) → the {@code onErrorResume} fallback
 *       returns an empty list so the caller never sees an exception.</li>
 *   <li>Empty body / no {@code data} field → empty list.</li>
 * </ol>
 *
 * Each test pins the endpoint URI so a future refactor that accidentally
 * swaps two endpoint constants in {@link UexProperties} would fail loud.
 */
class UexClientTest {
