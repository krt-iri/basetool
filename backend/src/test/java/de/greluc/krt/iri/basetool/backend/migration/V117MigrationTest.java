 ...
 * V117__drop_legacy_material_and_ship_type_columns.sql} — the R9 Step 4 destructive drop. Pins that
...
@SpringBootTest
@ActiveProfiles("test")
class V117MigrationTest {

  @Autowired private JdbcTemplate jdbcTemplate;

  @Test
  void v117IsRecordedAsApplied() {
    Integer applied =
        jdbcTemplate.queryForObject(
            "SELECT count(*) FROM flyway_schema_history WHERE version = '117' AND success = true",
            Integer.class);
    assertEquals(1, applied == null ? 0 : applied, "V117 must be applied successfully");
  }

  @Test
  void v117DropsMaterialIsManualEntry() {
    Integer count =
        jdbcTemplate.queryForObject(
            "SELECT count(*) FROM information_schema.columns "
                + "WHERE table_name = 'material' AND column_name = 'is_manual_entry'",
            Integer.class);
    assertEquals(0, count == null ? -1 : count, "material.is_manual_entry must be dropped by V117");
  }

  @Test
  void v117DropsShipTypeDescription() {
    Integer count =
        jdbcTemplate.queryForObject(
            "SELECT count(*) FROM information_schema.columns "
                + "WHERE table_name = 'ship_type' AND column_name = 'description'",
            Integer.class);
    assertEquals(0, count == null ? -1 : count, "ship_type.description must be dropped by V117");
  }
}
