...
class MissionManagerRoleTest {
    ...
    private String createMissionJson() throws Exception {
        MissionDto dto = new MissionDto(
                ...
                mission.getPlannedStartTime(), null, null, null,
                false,
    ...
    void missionDtoShouldHaveCanEditTrueForAdmin() throws Exception {
        mockMvc.perform(get("/api/v1/missions/" + mission.getId())
                        .with(jwt().authorities(new org.springframework.security.core.authority.SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andExpect(status().isOk())
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.canEdit").value(true));
    }
    ...
    void missionDtoShouldHaveCanEditFalseForRegularMember() throws Exception {
        mockMvc.perform(get("/api/v1/missions/" + mission.getId())
                        .with(jwt().jwt(builder -> builder.subject(otherMember.getId().toString()))))
                .andExpect(status().isOk())
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$.canEdit").value(false));
    }
    @Test
    void converterShouldAddMissionManagerRole() {
        ...
        org.junit.jupiter.api.Assertions.assertTrue(authorities.stream().anyMatch(a -> a.getAuthority().equals("ROLE_MISSION_MANAGER")),
