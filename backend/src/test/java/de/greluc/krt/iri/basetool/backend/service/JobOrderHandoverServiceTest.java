...
class JobOrderHandoverServiceTest {
    ...
    @Mock
    private JobOrderMaterialRepository jobOrderMaterialRepository;
    @Mock
    ...
    void createHandover_shouldReduceInventoryAmount_whenAmountIsSmallerThanStock() {
        ...
        // findById is called twice: once at the start and once after the loop to re-fetch the managed entity
    ...
    void createHandover_shouldDeleteInventoryItem_whenAmountIsFullyHandedOver() {
        ...
        de.greluc.krt.iri.basetool.backend.model.JobOrderHandover[] persistedHandover = new de.greluc.krt.iri.basetool.backend.model.JobOrderHandover[1];
        when(jobOrderHandoverRepository.save(any())).thenAnswer(i -> {
        ...
        // completeJobOrderWithinTransaction called with the re-fetched managed entity (same object in unit test)
    ...
    void createHandover_shouldThrowException_whenJobOrderIsNullOnInventoryItem() {
        ...
        JobOrderHandoverCreateDto createDto = new JobOrderHandoverCreateDto(Instant.now(), "swing-by", null, List.of(itemDto));
    ...
    void createHandover_shouldReduceBothJobOrderMaterialAmounts_whenTwoItemsHandedOver() {
        ...
        de.greluc.krt.iri.basetool.backend.model.Material material2 = new de.greluc.krt.iri.basetool.backend.model.Material();
        material2.setId(materialId2);

        de.greluc.krt.iri.basetool.backend.model.JobOrderMaterial jobOrderMaterial2 = new de.greluc.krt.iri.basetool.backend.model.JobOrderMaterial();
        ...
        JobOrderHandoverCreateDto createDto = new JobOrderHandoverCreateDto(Instant.now(), "swing-by", null, List.of(itemDto1, itemDto2));

        when(jobOrderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(inventoryItemRepository.findByIdForUpdate(inventoryId)).thenReturn(Optional.of(inventoryItem));
        when(inventoryItemRepository.findByIdForUpdate(inventoryId2)).thenReturn(Optional.of(inventoryItem2));
        when(jobOrderHandoverRepository.save(any())).thenAnswer(i -> i.getArgument(0));
        when(jobOrderHandoverMapper.toDto(any(JobOrderHandover.class))).thenReturn(mock(JobOrderHandoverDto.class));
        ...
        assertEquals(5.0, jobOrderMaterial.getAmount(), 0.0001,
                "First material's open amount must be reduced from 10.0 to 5.0");
        assertEquals(0.0, jobOrderMaterial2.getAmount(), 0.0001,
    ...
    void createHandover_shouldCallCompleteJobOrderWithinTransaction_whenAllMaterialsHandedOver() {
        ...
        JobOrderHandoverItemCreateDto itemDto = new JobOrderHandoverItemCreateDto(inventoryId, 10.0);
        JobOrderHandoverCreateDto createDto = new JobOrderHandoverCreateDto(
                Instant.now(), "swing-by", null, List.of(itemDto));
    ...
    @Test
    void createHandover_shouldCompleteOrder_whenLastRemainingMaterialHandedOverAfterPreviousPartialHandover() {
