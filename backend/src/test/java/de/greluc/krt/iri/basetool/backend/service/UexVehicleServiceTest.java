...
/**
 * Unit tests for {@link UexVehicleService}. UEX returns the full vehicle list
 * once per scheduler tick (see {@link UexScheduler}); the service has to
 * upsert matching ship types in our DB while gracefully tolerating partial
 * payloads.
 *
 * <p>The contract under test:
 * <ul>
 *   <li>An empty / null response from UEX must NOT wipe the local table —
 *     the service aborts early.</li>
 *   <li>Vehicles without a {@code name} must be skipped (foreign-key
 *     constraint violation otherwise).</li>
 *   <li>Manufacturers are looked up case-insensitively by company name; a
 *     missing manufacturer is non-fatal — the ship type still saves.</li>
 *   <li>A name match against an existing {@link ShipType} updates in place
 *     (preserving the entity id / version); no match creates a new row.</li>
 *   <li>Description is built up only from fields that are actually present
 *     so we don't pollute the DB with "Crew: null" lines.</li>
 * </ul>
 */
...
class UexVehicleServiceTest {
    ...
    void syncVehicles_skipsVehiclesWithoutName() {
        // Given
        UexVehicleDto noName = new UexVehicleDto(
                null, "Full Name", "Drake Interplanetary", 46, "1-4", "", "", 1, 0);
    ...
    void syncVehicles_skipsVehiclesWithBlankName() {
        // Given
        UexVehicleDto blank = new UexVehicleDto(
                "   ", "Full Name", "Drake", 46, "1", null, null, 1, 0);
    ...
    void syncVehicles_createsNewShipType_whenNameDoesNotExist() {
        ...
        UexVehicleDto cutlass = new UexVehicleDto(
                "Cutlass Black", "Drake Cutlass Black", "Drake Interplanetary",
                46, "1-2", "https://store/cutlass", "https://wiki/cutlass", 1, 0);
        ...
        when(shipTypeRepository.findByNameIgnoreCase("Cutlass Black"))
    ...
    void syncVehicles_updatesExistingShipType_inPlace_preservingId() {
        ...
        UexVehicleDto cat = new UexVehicleDto(
                "Caterpillar", "Drake Caterpillar", "Drake",
                576, "4-6", "https://store/cat", null, 1, 0);
        ...
        ArgumentCaptor<ShipType> saved = ArgumentCaptor.forClass(ShipType.class);
        verify(shipTypeRepository).save(saved.capture());
        assertSame(existing, saved.getValue(), "Existing entity must be mutated in place, NOT replaced");
        assertEquals(existingId, saved.getValue().getId());
        assertEquals(3L, saved.getValue().getVersion(), "Version must remain — JPA handles optimistic locking");
    ...
    void syncVehicles_savesWithoutManufacturer_whenLookupFails() {
        ...
        UexVehicleDto orphan = new UexVehicleDto(
                "Mystery Ship", null, "UnknownCorp", 10, null, null, null, 1, 0);
    ...
    void syncVehicles_skipsManufacturerLookup_whenCompanyNameMissing() {
        // Given
        UexVehicleDto noCompany = new UexVehicleDto(
                "Indie Ship", null, null, 5, null, null, null, 1, 0);
    ...
    void syncVehicles_buildsEmptyDescription_whenAllDescriptionFieldsBlank() {
        ...
        UexVehicleDto bare = new UexVehicleDto(
                "Bare Ship", null, null, null, null, null, null, 1, 0);
