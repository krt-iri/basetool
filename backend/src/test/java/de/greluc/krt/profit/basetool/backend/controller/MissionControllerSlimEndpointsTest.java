...
class MissionControllerSlimEndpointsTest {
  ...
  private Mission missionWithOtherUserAndGuest(UUID otherUserId, UUID guestParticipantId) {
    ...
    // ADR-0033: bob carries a free-text comment + the default PAYOUT preference so the outsider
  ...
  void addParticipantSlim_anonymousGuest_redactsOtherParticipantsPii() throws Exception {
    ...
    // ADR-0033 / REQ-SEC-021: the anonymous outsider roster must not carry a participant's
    // free-text comment or payout preference.
    org.junit.jupiter.api.Assertions.assertFalse(
        body.contains("bob-private-note"),
        "ADR-0033: anonymous outsider must not receive a participant's free-text comment");
    org.junit.jupiter.api.Assertions.assertFalse(
        body.contains("PAYOUT"),
        "ADR-0033: anonymous outsider must not receive a participant's payout preference");
  }
