...
- **Rollen- und staffel-/SK-übergreifende Tests** (Doku unter `docs/e2e-test/`, UC-08..11 + Rollen-Matrix): `RolePermissionsE2eTest` (Handover-Control für Member ausgeblendet, für Officer sichtbar), `CrossStaffelJobOrderE2eTest` (A bestellt, B liefert verknüpftes Inventar — im Auftrags-Kontext sichtbar, in A's Lager-View nicht), `CrossStaffelHandoverE2eTest` (Officer von A übergibt B's Material), `PublicMissionCrossStaffelE2eTest` (B-Mitglied sieht/öffnet A's öffentlichen Einsatz, nicht den internen), `SpecialCommandE2eTest` (SK-Anlage über die Admin-UI; SK als Job-Order-OrgUnit → 400). Mehrbenutzer-Orchestrierung über je einen `BrowserContext` pro Rolle/Staffel; zweite Staffel + SK per `BackendSeeder.createSquadron` / `createSpecialCommand`, Mitgliedschaften per `assignStaffelMembership`.
...
- **Job-Order-`responsibleOrgUnitId` + Profit-Eligibility:** `POST /api/v1/orders` verlangt für authentifizierte Aufrufer ein `responsibleOrgUnitId` (die bearbeitende Einheit), das auf eine **profit-eligible** OrgUnit auflöst (sonst 400); das frühere `creatingSquadronId` ist entfallen. Nur profit-eligible Einheiten stehen im „Bearbeitende Einheit"-Picker (V128, Default `FALSE`). Der Stack-Bootstrap (`E2eStackExtension`) schaltet IRIDIUM deshalb **einmalig** profit-eligible — **bevor** ein Seitenaufruf den 10-Minuten-Squadron-Katalog-Cache des Frontends füllt (sonst pinnt der Cache einen veralteten „nicht eligible"-Stand für den ganzen Lauf). `createJobOrder` schickt IRIDIUM als responsible **und** requesting; Inventar-Creates brauchen das nicht (Auto-Stempel bei genau einer Mitgliedschaft).
- **Fixed-Footer verdeckt Submit-Buttons (#276):** Das das-kartell-Designsystem macht den Footer `position: fixed; bottom: 0` (`.krt-footer`); `<main>` reserviert zwar `padding-bottom` = Footer-Höhe, aber Playwrights `click` scrollt das Element minimal an die untere Viewport-Kante — hinter den Footer — und re-scrollt bei jedem Versuch, sodass ein Koordinaten-Klick auf den Submit-Button langer Formulare (Einsatz, SK-Modal) vom Footer abgefangen wird (Timeout). Mehrere Auswege scheiterten je engine-spezifisch: `dispatchEvent('click')` ist untrusted und submittet nicht; ein `force`-Klick trifft den Footer obendrauf; `Enter` auf dem fokussierten Button aktiviert diese Submits auf WebKit nicht (Chromium/Firefox schon); `form.requestSubmit()` brach den App-Submit ab (keine Anlage). Lösung: `E2eSupport.clickSubmitClearingFooter` blendet den Footer per page-seitigem `evaluate` aus (`.krt-footer` → `display:none`; `evaluate` mit Funktion läuft unter der strikten CSP, anders als String-`eval`/`waitForFunction`) und klickt dann normal — ein echter, trusted Klick submittet mit voller Validierung, engine-übergreifend (Chromium/Firefox/WebKit).
- **Gecachte Job-Order-Material-Liste:** Das Frontend cached die Job-Order-Material-Liste (`getCached("/api/v1/materials/job-order")`). Im geteilten Stack füllt der erste Order-Seitenaufruf den Cache; später (von anderen Testklassen) geseedete Materialien fehlen dann im Dropdown. `JobOrderCreateE2eTest` wählt deshalb das erste angebotene Material (per Index), statt auf sein eigenes geseedetes zu bestehen.
- **Modale Create-Formulare:** Manche Create-Formulare liegen in einem Modal, nicht inline (z. B. SK über den `#add-sc-btn`-Button „Neues Spezialkommando"). Erst den Öffnen-Trigger klicken, dann die Modal-Felder füllen.
...
**CI-Workflows angelegt und grün** — destruktive Flows in `e2e.yml`, Smoke-Checks in `e2e-smoke.yml` (Aufteilung beim `smoke`-Job unten). Der erste echte CI-Lauf legte drei Linux-only-Probleme offen, die Docker Desktop lokal maskiert hatte; alle gefixt: (1) **Backend unhealthy** — `host.docker.internal:18080` (Keycloak, nur auf `127.0.0.1` published) ist über die `host-gateway`-Route auf einem Linux-Bridge nicht erreichbar → Netzwerk-Alias `host.docker.internal` auf `keycloak-dev` + `extra_hosts` im e2e-Override geleert, sodass der Issuer-Host auf den Container statt aufs Host-Gateway zeigt; (2) **WebKit startet nicht** (`DriverException`), da ubuntu-latest dessen Shared Libraries fehlen → `playwrightInstall --with-deps` auf CI-Linux (apt + passwordless sudo, lokal/Nicht-Linux ausgespart); (3) **CodeQL** meldete den Trust-all-`TrustManager` im `BackendSeeder` → durch ein gescoptes Trust-Store ersetzt, das nur das geladene Backend-Dev-Zertifikat akzeptiert (SAN deckt `localhost`, Hostname-Prüfung bleibt aktiv). Ergebnis: alle drei Browser grün, CodeQL grün.
...
- **Job `smoke`** (`@Tag("smoke")`, Staging) — in **eigenem Workflow `e2e-smoke.yml`**, der nur auf `schedule` + `workflow_dispatch` triggert (NICHT `pull_request`). Grund: als Matrix-Job, der bis zur Staging-Existenz per `if` übersprungen wird, expandiert GitHub die Matrix nicht und zeigte den rohen `Smoke (${{ matrix.browser }}, staging)`-Platzhalter als übersprungenen Check auf jedem PR — die Auslagerung hält ihn ganz aus den PR-Checks heraus. `if`-gegated auf die Repo-Variable `E2E_BASE_URL` (eine Variable statt Secret — nur so in `if:` lesbar); liegt brach, bis ein Staging-Host existiert. Credentials (`E2E_USERNAME`/`E2E_PASSWORD`) kommen aus Secrets **über die Umgebung** (maskiert), die die `smokeTest`-Task auf die `e2e.*`-System-Properties mappt — keine Secrets auf der Kommandozeile.
