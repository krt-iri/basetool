...
- **Fixed-Footer verdeckt Submit-Buttons (#276):** Das das-kartell-Designsystem macht den Footer `position: fixed; bottom: 0` (`.krt-footer`); `<main>` reserviert zwar `padding-bottom` = Footer-Höhe, aber Playwrights `click` scrollt das Element minimal an die untere Viewport-Kante — hinter den Footer — und re-scrollt bei jedem Versuch, sodass ein Koordinaten-Klick auf den Submit-Button langer Formulare (Einsatz, SK-Modal) vom Footer abgefangen wird (Timeout). `dispatchEvent('click')` ist untrusted und löst die Submit-Aktivierung nicht aus, ein `force`-Klick trifft den Footer obendrauf. Lösung: `E2eSupport.clickSubmitClearingFooter` fokussiert den Button und drückt `Enter` — Tastatur-Aktivierung erreicht das fokussierte Element ohne Hit-Test und submittet trusted.
