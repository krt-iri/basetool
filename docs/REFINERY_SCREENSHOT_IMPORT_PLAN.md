...
5. **Confidence semantics:** verbalized VLM self-confidence is measurably uncalibrated for OCR (≈59% abstention accuracy vs ≈68% for two-pass agreement — arXiv 2511.19806). `confidence` is now **derived**: exact-match agreement across two reads (varying crop padding/scale) blended with checksum validation against the panel's `IN MANIFEST`/`TO REFINE` totals (semantics verified in Phase 0). The §5.4 UI thresholds are unchanged — they just receive a meaningful number.
      ...
      "goods": [
        {
          ...
          "confidence": 0.95,             // 0..1, DERIVED per-row confidence (two-pass agreement + checksum) — never the model's verbalized self-estimate
...
- **Derived confidence** — per-row confidence computed from two-pass read agreement + the header-total checksum (never the model's verbalized self-estimate).
