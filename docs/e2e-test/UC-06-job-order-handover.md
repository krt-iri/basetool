...
- **Admin-„alle Staffeln"-Modus:** Der Seeder-Token ist ein Admin ohne aktiven Pin; `POST /api/v1/orders` verlangt dann ein explizites `creatingSquadronId` (400 sonst). Der UI-Flow funktioniert ohne, weil das Frontend den Aktiv-Kontext-Header relayt.
