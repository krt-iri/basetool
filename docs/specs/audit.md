...
builder — `AuditDetails.of("section", "full").with("status", status).toString()` — instead of being
...
`UUID`/number/boolean/`null` as its `toString()`), so the emitted trail is unchanged. It
deliberately does **not** throw on value *content*: `record(...)` runs inside the business
...
trail); those keep their bespoke composition and are not expressed through the builder.
