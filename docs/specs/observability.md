...
> **Owner area:** OBS · **Related:** [`security-and-access.md`](security-and-access.md), [`org-unit-tenancy.md`](org-unit-tenancy.md), [ADR-0064](../adr/0064-monitoring-stack-prometheus-grafana.md), monitoring epic [#936](https://github.com/krt-profit/basetool/issues/936)
...
[#936](https://github.com/krt-profit/basetool/issues/936), ADR-0064) builds on these
...
scrape (epic #936, ADR-0064). The endpoint is **never public**:
...
- **NPM access logs, NPM admin-UI stdout, and SSH/host-auth logs** — ingested **including
  ...
  the GFS retention cannot silently extend those 31 days (ADR-0064).
...
- **Transport:** all app/Keycloak/NPM edges stay HTTPS — Prometheus scrapes the three
  ...
  **Inside** the isolated monitoring networks (Prometheus→exporters, Grafana→datasources,
  ...
  that actually creates those networks. Rationale and residual risk live in ADR-0064.
