...
public class AdminTerminalsPageController {
    ...
    public String listData(Model model) {
        try {
            ...
            if (terminalsPage != null && terminalsPage.content() != null) {
                terminals = terminalsPage.content().stream()
                        .map(m -> new TerminalDto(
                                parseUuid(m.get("id")),
                ...
                terminals.sort(Comparator.comparing(t -> t.name() == null ? "" : t.name(), String.CASE_INSENSITIVE_ORDER));
            }
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'OFFICER')")
    public String toggleTerminalVisibility(@PathVariable @NotNull UUID id, @RequestParam boolean hidden, RedirectAttributes redirectAttributes) {
        try {
            TerminalDto currentTerminal = backendApiClient.get("/api/v1/terminals/" + id, TerminalDto.class);
            TerminalDto body = new TerminalDto(
                    id, 
