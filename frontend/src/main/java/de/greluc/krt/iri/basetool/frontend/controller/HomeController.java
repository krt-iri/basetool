...
public class HomeController {
    ...
    @GetMapping("/")
    public String home(Model model, @AuthenticationPrincipal OidcUser principal, HttpSession session) {
        // Fetch Next Mission
        try {
            Map<String, Object> nextMission = backendApiClient.get(
        ...
        if (principal != null) {
            ...
            try {
                de.greluc.krt.iri.basetool.frontend.model.dto.UserDto currentUser = backendApiClient.get("/api/v1/users/me", de.greluc.krt.iri.basetool.frontend.model.dto.UserDto.class);
                model.addAttribute("currentUser", currentUser);
                
                // Fetch Public Announcement
                Map<String, Object> announcement = backendApiClient.get(
                        "/api/v1/announcement",
                        new ParameterizedTypeReference<Map<String, Object>>() {}
                );
                ...
                if (announcement != null && announcement.containsKey("id")) {
                    ...
                    if (currentUser.lastReadAnnouncementId() == null || !currentUser.lastReadAnnouncementId().toString().equals(announcementId)) {
                        unread = true;
                    }
                }
    ...
    public String markAnnouncementAsRead(@org.springframework.web.bind.annotation.RequestParam String id) {
        try {
