 ...
 *
 * <p>Forwards PDF download and preview requests to the backend via the authenticated
 * {@link WebClient} (which automatically attaches the OAuth2 token), and streams
 * the PDF bytes back to the browser.
 */
@RestController
...
public class JobOrderHandoverReportProxyController {
    ...
    public ResponseEntity<byte[]> downloadHandoverReport(
        ...
        try {
            ...
            log.warn("Handover report proxy: backend returned {} — {}", e.getStatusCode(), e.getMessage());
    ...
    public ResponseEntity<byte[]> previewHandoverReport(
        ...
        try {
            ...
            log.warn("Handover report preview proxy: backend returned {} — {}", e.getStatusCode(), e.getMessage());
            throw new ResponseStatusException(e.getStatusCode(), e.getMessage());
        } catch (ResponseStatusException e) {
            throw e;
        } catch (Exception e) {
