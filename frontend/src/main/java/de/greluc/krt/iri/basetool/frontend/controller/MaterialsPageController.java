...
public class MaterialsPageController {
    ...
    public record MaterialRow(String materialName, Boolean isIllegal, Boolean isVolatileQt, Boolean isVolatileTime, Map<String, MaterialMatrixItemDto> prices) implements Comparable<MaterialRow> {
        @Override
    ...
    public String listMaterials(Model model) {
        try {
            PageResponse<MaterialPriceOverviewDto> page = backendApiClient.get(
            ...
            for (MaterialPriceOverviewDto mat : materials) {
                String kind = mat.category() != null && mat.category().name() != null && !mat.category().name().isBlank() ? mat.category().name() : "Unsortiert";
                materialsByKind.computeIfAbsent(kind, k -> new ArrayList<>()).add(mat);
            }
            // Sort items within each kind alphabetically by name (already sorted from API, but just to be sure)
            materialsByKind.values().forEach(list -> list.sort(Comparator.comparing(MaterialPriceOverviewDto::name, String.CASE_INSENSITIVE_ORDER)));
    ...
    public String getMatrixOverview(
            ...
            Model model) {
        try {
            PageResponse<MaterialMatrixItemDto> page = backendApiClient.get(
            ...
            Set<String> starSystems = items.stream()
            ...
            Set<String> materialNames = items.stream()
            ...
            for (MaterialMatrixItemDto item : items) {
                ...
                kindByMaterial.put(item.materialName(), item.category() != null && item.category().name() != null && !item.category().name().isBlank() ? item.category().name() : "Unsortiert");
                ...
                materialPrices.computeIfAbsent(item.materialName(), k -> new HashMap<>())
                        .put(item.terminalName(), item);
            }

            Map<String, List<MaterialRow>> rowsByKind = new TreeMap<>();
            for (Map.Entry<String, Map<String, MaterialMatrixItemDto>> entry : materialPrices.entrySet()) {
                ...
                rowsByKind.computeIfAbsent(kind, k -> new ArrayList<>())
                        .add(new MaterialRow(matName, isIllegal, isVolatileQt, isVolatileTime, entry.getValue()));
            }
            rowsByKind.values().forEach(list -> list.sort(Comparator.comparing(MaterialRow::materialName, String.CASE_INSENSITIVE_ORDER)));
    ...
    public String getMaterialDetail(@PathVariable @NotNull UUID id, Model model) {
        try {
            MaterialDto material = backendApiClient.get(
                    "/api/v1/materials/" + id,
                    MaterialDto.class
            );
            model.addAttribute("material", material);

            PageResponse<MaterialPriceDto> pricesPage = backendApiClient.get(
