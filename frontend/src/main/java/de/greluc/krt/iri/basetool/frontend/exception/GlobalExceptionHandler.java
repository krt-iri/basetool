 ...
 *
 * <p>Translates {@link BackendServiceException} (raised by the WebClient layer when
 * the backend returns an RFC7807 Problem+JSON response or when a Resilience4j
 * fallback fires) into localized, user-facing messages using
 * {@link MessageSource}. The stable Problem {@code code} (e.g.
 * {@code OPTIMISTIC_LOCK}, {@code ACCESS_DENIED}, {@code VALIDATION_FAILED}) is
 * mapped onto a well-defined set of {@code error.*} message keys — no hardcoded
 * user-facing strings are emitted (AGENTS.md: DYNAMIC TRANSLATION ONLY).
 *
 * <p>AJAX/JSON requests receive a compact JSON body so that the client-side
 * {@code window.showError(problem)} function can render a KRT-styled toast,
 * while regular navigation requests render the error page with a localized title
 * and explanation.
 */
@ControllerAdvice
...
public class GlobalExceptionHandler {
    ...
    public Object handleBackendServiceException(@NotNull BackendServiceException ex,
        ...
        String localizedTitle = resolve(titleKeyForStatus(status), locale, resolve(DEFAULT_TITLE_KEY, locale, "Error"));

        log.warn("Backend error propagated to user: code={}, status={}, correlationId={}, uri={}",
                ex.getProblemCode(), status.value(), ex.getCorrelationId(), request.getRequestURI());
        ...
        if (wantsJson(request)) {
            ...
            body.put("reloadHint", ex.getProblemCode().equals("OPTIMISTIC_LOCK")
                    || ex.getProblemCode().equals("CONFLICT"));
    ...
    public String handleNoResourceFoundException(@NotNull NoResourceFoundException e, @NotNull Model model) {
        Locale locale = LocaleContextHolder.getLocale();
        model.addAttribute("error", resolve("error.404.title", locale, "Not Found"));
        model.addAttribute("message", resolve("error.404.message", locale, "The requested resource could not be found."));
    ...
    public Object handleTypeMismatch(@NotNull MethodArgumentTypeMismatchException ex,
                                     @NotNull Model model,
                                     @NotNull HttpServletRequest request) {
        // Do NOT log ex.getValue() - request parameter values may carry PII.
        log.warn("Frontend type mismatch for {} {} [parameter={}, targetType={}]",
                request.getMethod(), request.getRequestURI(), ex.getName(),
                ex.getRequiredType() != null ? ex.getRequiredType().getSimpleName() : "n/a");
        Locale locale = LocaleContextHolder.getLocale();
        String message = resolve("error.validation.failed", locale, "Invalid parameter " + ex.getName());
        if (wantsJson(request)) {
            Map<String, Object> body = new LinkedHashMap<>();
    ...
    /**
     * Translates Spring Security authorization failures (raised by {@code @PreAuthorize}
     * checks within controllers) into a 403 response page or a JSON body for AJAX
     * callers, instead of letting them fall through to the generic 500 handler.
     * Without this mapping, a missing role would surface to the user as an opaque
     * "Internal Server Error" — which both hides the real cause and contradicts the
     * standard semantics for HTTP 403.
     */
    @ExceptionHandler({
    ...
    public String handleException(@NotNull Exception e, @NotNull Model model, @NotNull HttpServletRequest request) {
        Locale locale = LocaleContextHolder.getLocale();
        String status = "500";
        ...
        log.error("Unexpected frontend error for {} {} [exception={}]: {}",
                request.getMethod(), request.getRequestURI(),
    ...
    private @NotNull String resolve(@NotNull String key, @NotNull Locale locale, @NotNull String fallback) {
        try {
