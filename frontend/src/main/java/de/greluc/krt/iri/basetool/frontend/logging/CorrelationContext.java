 ...
 * re-reading the original {@code HttpServletRequest}. Populated by {@link CorrelationIdFilter}
 * at the beginning of every request and cleared in the matching {@code finally} block.
 *
 * <p>Virtual threads and reactive pipelines copy the owning thread's thread-locals on submission,
 * so the value is visible inside {@code WebClient} exchange filters that run on the request
 * thread. Calls escaping to a different scheduler must pass the id explicitly.</p>
 */
