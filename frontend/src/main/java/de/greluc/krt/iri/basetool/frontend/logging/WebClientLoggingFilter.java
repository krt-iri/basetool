 ...
 *       (configurable via {@link LoggingProperties#getCorrelationIdHeader()}) into outbound
 *       backend calls so the backend log line shares the same id.</li>
 *   <li><b>Structured call logging</b> – logs one line per outbound call with method, host, path,
 *       status and elapsed time. Slow calls (above
 *       {@link LoggingProperties#getSlowBackendCallThresholdMs()}) escalate to WARN so they can
 *       be flagged in dashboards. Network-level failures are logged at WARN as well, with the
 *       exception class and message only (no stack trace – the exception is re-thrown and the
 *       frontend {@code GlobalExceptionHandler} decides on user-facing behaviour).</li>
 * </ul>
 *
 * <p>Query strings are intentionally excluded from the log line because they may carry PII such
 * as filter expressions containing email fragments or user ids of other users (see AGENTS.md:
 * no information disclosure).</p>
 */
@Slf4j
@Component
...
public class WebClientLoggingFilter {
    ...
    private void logCall(@NotNull String method, @NotNull String host, @NotNull String path,
                         int status, long startNanos) {
    ...
    private void logError(@NotNull String method, @NotNull String host, @NotNull String path,
                          @NotNull Throwable err, long startNanos) {
        long durationMs = (System.nanoTime() - startNanos) / 1_000_000L;
        log.warn("Backend call {} {}{} failed after {} ms: {}: {}", method, host, path, durationMs,
