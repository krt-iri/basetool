 ...
 * via the authenticated {@link WebClient} (which automatically attaches the OAuth2 token), and
...
public class HangarDeleteAllProxyController {
  ...
  public ResponseEntity<Void> deleteAllShips() {
    try {
      webClient.delete().uri("/api/v1/hangar/ships").retrieve().toBodilessEntity().block();
      return ResponseEntity.noContent().build();
    } catch (WebClientResponseException e) {
      log.warn(
          "Delete-all-ships proxy: backend returned {} — {}", e.getStatusCode(), e.getMessage());
