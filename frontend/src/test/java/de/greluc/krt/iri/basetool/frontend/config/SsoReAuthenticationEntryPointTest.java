...
class SsoReAuthenticationEntryPointTest {
    ...
    @Test
    void commence_shouldRedirectToSilentSsoAndSetAttemptedCookie_whenNoAttemptCookiePresent() throws Exception {
        // Given
        ...
        assertTrue(redirectedUrl.contains("/oauth2/authorization/keycloak"), "Should redirect to OAuth2 authorization endpoint");
    ...
    @Test
    void commence_shouldRedirectToLoginPageAndClearCookie_whenAttemptCookieAlreadyPresent() throws Exception {
        // Given
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/dashboard");
        ...
        String redirectedUrl = response.getRedirectedUrl();
        assertNotNull(redirectedUrl);
        assertTrue(redirectedUrl.contains("/oauth2/authorization/keycloak"), "Should redirect to login page");
        assertFalse(redirectedUrl.contains("prompt=none"), "Should NOT include prompt=none on second attempt");
    ...
    void commence_shouldTriggerSilentSso_whenLegitimateAppPathRequested(String appUri) throws Exception {
        // Given
        MockHttpServletRequest request = new MockHttpServletRequest();
        ...
        assertTrue(redirectedUrl.contains("prompt=none"), "Legitimate app path should use silent SSO. URI=" + appUri);
