...
/**
 * Unit tests for {@link HangarDeleteAllProxyController}. Drives the real
 * WebClient against {@link MockWebServer} so every exception-handling branch
 * is exercised end-to-end without a Spring context.
 *
 * <p>Coverage points:
 * <ul>
 *   <li>Happy path: backend returns 204 → controller returns 204.</li>
 *   <li>Backend client error (4xx) → controller re-throws as
 *       {@link ResponseStatusException} with the same status.</li>
 *   <li>Backend server error (5xx) → same passthrough behaviour.</li>
 *   <li>Network failure (server shutdown mid-request) → controller wraps in
 *       a 500 {@code ResponseStatusException}.</li>
 * </ul>
 */
