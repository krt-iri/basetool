...
class MissionPageControllerTest {
    ...
    void addParticipant_WithUserId_Authenticated_ShouldCallWebClient() {
        ...
        // Assert
        assertEquals("redirect:/missions/" + id, view);
        verify(backendApiClient).post(eq("/api/v1/missions/" + id + "/participants/add"), any(), eq(Void.class), eq(false));
    }
    @Test
    void deleteParticipant_ShouldCallWebClient() {
        ...
        // Act
        String view = controller.deleteParticipant(id, participantId, user, mock(RedirectAttributes.class));

        // Assert
        assertEquals("redirect:/missions/" + id, view);
        verify(backendApiClient).delete(eq("/api/v1/missions/" + id + "/participants/" + participantId), eq(Void.class), eq(false));
    }
    ...
    void deleteParticipant_Anonymous_ShouldCallPublicWebClient() {
        ...
        // Act
        String view = controller.deleteParticipant(id, participantId, null, mock(RedirectAttributes.class));

        // Assert
        assertEquals("redirect:/missions/" + id, view);
        verify(backendApiClient).delete(eq("/api/v1/missions/" + id + "/participants/" + participantId), eq(Void.class), eq(true));
    }
    ...
    void updateParticipant_ShouldCallWebClient() {
        ...
        // Assert
        assertEquals("redirect:/missions/" + id, view);
        verify(backendApiClient).put(eq("/api/v1/missions/" + id + "/participants/" + participantId), any(), eq(Void.class), eq(false));
    }
    ...
    void updateParticipant_Anonymous_ShouldCallPublicWebClient() {
        ...
        // Assert
        assertEquals("redirect:/missions/" + id, view);
        verify(backendApiClient).put(eq("/api/v1/missions/" + id + "/participants/" + participantId), any(), eq(Void.class), eq(true));
    }
    ...
    void missionDetail_ShouldFetchMissionAndFilteredJobTypes() {
        ...
        when(backendApiClient.getCached(anyString(), any(ParameterizedTypeReference.class), eq(true))).thenReturn(Collections.emptyList());
        ...
        verify(backendApiClient).get(eq("/api/v1/missions/" + id), any(ParameterizedTypeReference.class), eq(true));
        verify(backendApiClient).getCached(eq("/api/v1/job-types?archetype=MISSION&size=1000"), any(ParameterizedTypeReference.class), eq(true));
        verify(backendApiClient).getCached(eq("/api/v1/job-types?archetype=CREW&size=1000"), any(ParameterizedTypeReference.class), eq(true));
        verify(backendApiClient).getCached(eq("/api/v1/squadrons?size=1000"), any(ParameterizedTypeReference.class), eq(true));
    }
    ...
    void missionDetail_Guest_ShouldNotFetchFinanceOrRefineryOrders() {
        ...
        when(backendApiClient.get(eq("/api/v1/missions/" + id), any(ParameterizedTypeReference.class), eq(true))).thenReturn(mission);
        when(backendApiClient.get(eq("/api/v1/missions/" + id + "/units?size=1000"), any(ParameterizedTypeReference.class), eq(true)))
                .thenReturn(new de.greluc.krt.iri.basetool.frontend.model.dto.PageResponse<>(Collections.emptyList(), 0, 10, 0, 0, Collections.emptyList()));
        when(backendApiClient.getCached(anyString(), any(ParameterizedTypeReference.class))).thenReturn(Collections.emptyList());
        when(backendApiClient.getCached(anyString(), any(ParameterizedTypeReference.class), anyBoolean())).thenReturn(Collections.emptyList());

        // Act
        controller.missionDetail(id, model, null);

        // Assert
        verify(backendApiClient, never()).get(contains("/finance-entries"), any(ParameterizedTypeReference.class), anyBoolean());
        verify(backendApiClient, never()).get(contains("/refinery-orders"), any(ParameterizedTypeReference.class), anyBoolean());
        verify(backendApiClient, never()).get(contains("/finance-entries"), any(Class.class), anyBoolean());
    }
}
