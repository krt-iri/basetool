 ...
 * <ul>
 *     <li>the legacy "Filtern" submit button is no longer rendered,</li>
 *     <li>the reset control and the AJAX results container are present,</li>
 *     <li>the {@code fragment=results} query parameter returns only the
 *         results fragment (no outer page chrome).</li>
 * </ul>
 */
@SpringBootTest
...
class MissionsLiveFilterPageTest {
    ...
    void setup() {
        ...
        PageResponse<MissionListDto> emptyPage = new PageResponse<>(
                List.<MissionListDto>of(), 0, 20, 0L, 0, List.of());
        when(backendApiClient.get(anyString(),
    ...
    void missionsPage_WithFragmentResults_ShouldReturnOnlyResultsFragment() throws Exception {
        mockMvc.perform(get("/missions").param("fragment", "results"))
                .andExpect(status().isOk())
