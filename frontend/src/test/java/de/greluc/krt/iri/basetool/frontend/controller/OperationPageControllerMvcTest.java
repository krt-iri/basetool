...
/**
 * Verifies that the operations index and operation-detail templates render
 * status values through the i18n bundle instead of leaking the raw backend
 * enum strings. The i18n bug was: the create/edit dropdowns went through
 * {@code operation.status.planned} etc., but the displayed status (in the
 * table, the detail box, and the embedded missions table) was rendered as
 * {@code th:text="${op.status}"}, dumping the raw enum name into the page.
 * These tests pin the fix: send an Operation/Mission with status
 * {@code PLANNED}/{@code COMPLETED}/{@code CANCELLED} through the page and
 * assert the rendered HTML contains the German translation, not the raw
...
class OperationPageControllerMvcTest {
    ...
    void operationsList_rendersOperationStatusViaI18n_inGerman() throws Exception {
        ...
        PageResponse<OperationDto> page = new PageResponse<>(
                List.of(op), 0, 20, 1L, 1, List.of("createdAt,desc"));
        when(backendApiClient.get(startsWith("/api/v1/operations?"),
                any(ParameterizedTypeReference.class), anyBoolean()))
                .thenReturn(page);

        mockMvc.perform(get("/operations").locale(Locale.GERMAN))
    ...
    void operationsList_rendersOperationStatusViaI18n_inEnglish() throws Exception {
        ...
        mockMvc.perform(get("/operations").locale(Locale.ENGLISH))
                .andExpect(status().isOk())
    ...
    void operationDetail_readOnlyUser_seesDisabledFormAndNoSaveButton() throws Exception {
        ...
        mockMvc.perform(get("/operations/" + opId).locale(Locale.GERMAN))
                .andExpect(status().isOk())
    ...
    private void stubDetailEndpoints(UUID opId, OperationDto operation) {
        when(backendApiClient.get(eq("/api/v1/operations/" + opId),
                eq(OperationDto.class), anyBoolean()))
                .thenReturn(operation);
        when(backendApiClient.get(contains("/api/v1/missions/search?operationId=" + opId),
                any(ParameterizedTypeReference.class), anyBoolean()))
                .thenReturn(new PageResponse<>(List.<MissionListDto>of(), 0, 10, 0L, 0, List.of()));
        when(backendApiClient.get(eq("/api/v1/operations/" + opId + "/finances"),
                eq(OperationFinanceDto.class), anyBoolean()))
                .thenReturn(new OperationFinanceDto(opId, BigDecimal.ZERO, List.of()));
        when(backendApiClient.get(eq("/api/v1/operations/" + opId + "/payouts"),
                any(ParameterizedTypeReference.class), anyBoolean()))
                .thenReturn(List.of());
    }
    ...
    void operationDetail_translatesBothOperationAndMissionStatus() throws Exception {
        ...
        // raw enum value.
        MissionListDto mission = new MissionListDto(
                UUID.randomUUID(), "Aborted Mission", null, null, "CANCELLED",
                null, null, null, null, null, null, null, 0L);
        PageResponse<MissionListDto> missionsPage = new PageResponse<>(
                List.of(mission), 0, 10, 1L, 1, List.of("plannedStartTime,asc"));
        when(backendApiClient.get(contains("/api/v1/missions/search?operationId=" + opId),
                any(ParameterizedTypeReference.class), anyBoolean()))
