...
/**
 * Pure-Mockito unit tests for {@link ProfileController}. The full
 * Thymeleaf-rendering MVC integration is covered separately in
 * {@code ProfileControllerMvcTest}; here we focus on:
 * <ul>
 *   <li>The unauthenticated branch (no principal → redirect home).</li>
 *   <li>OIDC-claim-vs-backend precedence: backend overwrites token claims.</li>
 *   <li>Join-date parsing and the months-in-squadron computation.</li>
 *   <li>POST /profile/description happy / validation-error / optimistic-
 *       lock / generic-error branches — these decide which toast the user
 *       sees and whether the form re-renders or redirects.</li>
 *   <li>Multi-valued OIDC claim handling (Keycloak returns lists for some
 *       custom claims).</li>
 * </ul>
 */
...
class ProfileControllerTest {
    ...
    void profile_authenticated_populatesModelFromTokenAndBackendOverride() {
        ...
        Map<String, Object> backendUser = Map.of(
                "rank", 7,
                "description", "From-Backend",
                "displayName", "Backend-DN",
                "version", 4L,
                "joinDate", "2024-01-15");
        when(backendApiClient.<Map<String, Object>>get(eq("/api/v1/users/me"), any(org.springframework.core.ParameterizedTypeReference.class)))
        ...
        assertEquals("https://kc.example.com/realms/iri/account",
                model.getAttribute("keycloakAccountUrl"));
        // ProfileDescriptionForm is seeded from current model state
        ProfileDescriptionForm form = (ProfileDescriptionForm) model.getAttribute("profileDescriptionForm");
        assertNotNull(form);
    ...
    void profile_multiValueClaim_returnsFirstElement() {
        ...
        when(backendApiClient.<Map<String, Object>>get(any(String.class), any(org.springframework.core.ParameterizedTypeReference.class))).thenReturn(null);
    ...
    void profile_authenticated_backendUserMissingDescriptionKey_keepsToken() {
        ...
        when(backendApiClient.<Map<String, Object>>get(any(String.class), any(org.springframework.core.ParameterizedTypeReference.class)))
    ...
    void profile_invalidJoinDateFormat_isSilentlyIgnored() {
        ...
        when(backendApiClient.<Map<String, Object>>get(any(String.class), any(org.springframework.core.ParameterizedTypeReference.class))).thenReturn(backendUser);
    ...
    void updateDescription_happyPath_putsAndRedirectsWithSuccessToast() {
        ...
        String view = controller.updateDescription(form, bindingResult, new ConcurrentModel(), principal, redirectAttributes);

        assertEquals("redirect:/profile", view);

        ArgumentCaptor<Map<String, Object>> bodyCap =
                ArgumentCaptor.forClass(Map.class);
        verify(backendApiClient).put(eq("/api/v1/users/me/description"), bodyCap.capture(), eq(Void.class));
    ...
    void updateDescription_nullFields_sendEmptyStrings() {
        ...
        controller.updateDescription(form, bindingResult, new ConcurrentModel(), principal, redirectAttributes);
    ...
    void updateDescription_optimisticLockConflict_setsConcurrencyToast() {
        ...
        BackendServiceException conflict = org.mockito.Mockito.spy(new BackendServiceException(
        ...
        doThrow(conflict).when(backendApiClient).put(eq("/api/v1/users/me/description"), any(), eq(Void.class));
    ...
    void updateDescription_genericBackendException_setsGenericErrorToast() {
        when(bindingResult.hasErrors()).thenReturn(false);
        BackendServiceException other = new BackendServiceException(
                "boom", null, 500, "UNKNOWN", null, List.of(), null);
