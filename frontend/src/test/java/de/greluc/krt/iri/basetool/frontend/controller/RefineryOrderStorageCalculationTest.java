...
class RefineryOrderStorageCalculationTest {
    ...
    void testStoreFormCalculationForPiece() {
        ...
        MaterialDto pieceMaterial = new MaterialDto(UUID.randomUUID(), 1, "PieceMat", "TYPE", "PIECE", null, null, null, false, false, false, 0L);
        RefineryGoodDto good = new RefineryGoodDto(UUID.randomUUID(), pieceMaterial, 100, pieceMaterial, 15, 100, null);
        RefineryOrderDto orderDto = new RefineryOrderDto(orderId, null, null, null, null, 60L, 0.0, null, List.of(good), RefineryOrderStatus.OPEN, 0L);
    ...
    void testStoreFormCalculationForScu() {
        UUID orderId = UUID.randomUUID();
        MaterialDto scuMaterial = new MaterialDto(UUID.randomUUID(), 2, "ScuMat", "TYPE", "SCU", null, null, null, false, false, false, 0L);
        ...
        when(backendApiClient.get(eq("/api/v1/settings/refinery.rounding.mode"), any(Class.class))).thenReturn(settingDto);
        when(oidcUser.getSubject()).thenReturn(UUID.randomUUID().toString());
