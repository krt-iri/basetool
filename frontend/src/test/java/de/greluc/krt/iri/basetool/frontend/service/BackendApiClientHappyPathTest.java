...
/**
 * Happy-path coverage for every HTTP verb and overload on
 * {@link BackendApiClient}, driven by {@link MockWebServer}.
 *
 * <p>The companion suite {@code BackendApiClientResilienceTest} covers the
 * error / unwrap branches via plain Mockito; {@code BackendApiClientProblemJsonTest}
 * covers RFC 7807 decoding. This file plugs the remaining gap: the success
 * path of every public method (cached / uncached, {@code Class<T>} /
 * {@code ParameterizedTypeReference<T>}, public / authenticated client,
 * POST/PUT/PATCH with and without a body, DELETE).
 *
 * <p>A single shared {@link MockWebServer} is used; the {@code isPublic}
 * flag selects between two WebClient instances that both point at the same
 * server so we can observe which path was taken (the server records every
 * request).
 */
class BackendApiClientHappyPathTest {
    ...
    void setUp() throws Exception {
        ...
        webClient = WebClient.builder()
                ...
                .build();
        publicWebClient = WebClient.builder()
                .baseUrl(server.url("/").toString())
    ...
    void getCached_withParameterizedType_parsesBody() {
        ...
        List<Integer> result = client.getCached("/api/v1/cached-list", new ParameterizedTypeReference<>() {});
    ...
    void getCached_withParameterizedType_andIsPublicTrue_usesPublicClient() throws Exception {
        ...
        client.getCached("/api/v1/cached-list", new ParameterizedTypeReference<List<Integer>>() {}, true);
